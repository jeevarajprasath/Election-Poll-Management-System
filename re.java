...
class re extends JFrame implements ActionListener,KeyListener

{

JTextField t2,t3,t4,t5,t6,t7,t8,t9,t10,tuser,tpwd;
JButton b1,b2,b3;
JLabel l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,l14,luser,lpwd,m,ct,tn,
l15,l16,ln,lt,lb,lr1,lr2,lr3,lr4,lr5,lr6,lr7,lr8,lr9,lr10,lr11,lr12,lr13,lr14,lman,t1;
JTextArea a1,a2;
ButtonGroup group;
ButtonModel bm;
//Checkbox g1,g2;
...
JRadioButton r1,r2;
  ...
  re()
 {
   ...
   ResultSet rs3 = st.executeQuery("select max(Voterno) from voter_reg");
      cno=1;
   ...
   //System.out.println(cno);
    ...
    lt=new JLabel("VOTER REGISTRATION FORM");    			//Textbox admission form
    l1=new JLabel("Voter  no");				//Labels
    l2=new JLabel("Name ");
    l3=new JLabel("Gender");
    l4=new JLabel("Date of Birth");
    l5=new JLabel("Birth Place");
    l6=new JLabel("Name of Municipality");
    l7=new JLabel("Name of Local Constituency");
    ln=new JLabel("Nationality");
    ...
    m=new JLabel("Nashik");
    ct=new JLabel("Nashik Municipal Corporation");
    tn=new JLabel("Indian");
   /* lr1=new JLabel("*");
    lr2=new JLabel("*");
    lr3=new JLabel("*");
    lr4=new JLabel("*");
...
/*    cg=new CheckboxGroup();				//Radio buttons
   ...
   l1.setBounds(30,90,90,20);      
   add(l1);

   t1.setFont(new Font("TimesRoman",Font.BOLD,16));
   t1.setBounds(125,90,30,25);
   t1.setBorder(border);
   add(t1);
 
   luser.setFont(new Font("TimesRoman",Font.BOLD,16));
   //lr1.setBounds(10,90,90,20);    
   //add(lr1);
   //lr1.setForeground(Color.red);
   luser.setBounds(270,90,110,20);      
   add(luser);

   tuser.setFont(new Font("TimesRoman",Font.BOLD,16));
   tuser.setBounds(382,90,100,25);
   tuser.setBorder(border);
   add(tuser);

   lpwd.setFont(new Font("TimesRoman",Font.BOLD,16));
   //lr1.setBounds(10,90,90,20);    
   //add(lr1);
   //lr1.setForeground(Color.red);
   lpwd.setBounds(497,90,110,20);      
   add(lpwd);

   tpwd.setFont(new Font("TimesRoman",Font.BOLD,16));
   tpwd.setBounds(613,90,100,25);
   tpwd.setBorder(border);
   add(tpwd);
 ...
 add(c3); 
  ...
  ln.setBounds(270,280,150,20);
  add(ln);

  tn.setFont(new Font("TimesRoman",Font.BOLD,16));
  tn.setBounds(386,280,100,20);
  tn.setBorder(border);
  add(tn);
  ...
  m.setFont(new Font("TimesRoman",Font.BOLD,16));
  m.setBounds(30,360,200,25);
  m.setBorder(border);
  add(m);
  ...
  ct.setFont(new Font("TimesRoman",Font.BOLD,16));
  ct.setBounds(30,430,300,25);
  ct.setBorder(border);
  add(ct);
...
//t2.setText("");
     ...
     public void actionPerformed(ActionEvent ae)
  {
       ...
       String tf2,tf3,tf4,tf5,tfn,tf6,tf9,tf10,tf7,tf11,tf12;
        ...
        if(ae.getSource()==b1)
    {
        ...
        if(t1.getText().equals("") || t2.getText().equals("")|| t3.getText().equals("")|| tn.getText().equals("")||bm==null||a1.getText().equals("")||a2.getText().equals("")||t6.getText().equals("")||t7.getText().equals("") ||
...

     {
      ...
      try
      {
      //tf1=Integer.parseInt(t1.getText());//id
      tf2=t2.getText();//name
      ...
      //tfn=tn.getText();//Nationality
      //tf4=t4.getText();//Name_of_Municipality
      //tf5=t5.getText();//Name of Loacl constituency
      ...
      tf11=tuser.getText();//username
      tf12=tpwd.getText();//password.
     String dob=c3.getSelectedItem()+"-"+c2.getSelectedItem()+"-"+c1.getSelectedItem();
 


   PreparedStatement ps=con.prepareStatement("insert into voter_reg(Name,Gender,DOB,Birthplace,Nationality,Address,street_address,Locality,Pincode,Phone_no,Email_id,Adharcard_no,status,username,password,Voterno) values(?,?,?,?,'Indian',?,?,?,?,?,?,?,0,?,?,?)");
        //ps.setInt(1,tf1);
        ps.setString(1,tf2.toLowerCase());
        ps.setString(2,rmale.toLowerCase());
        ps.setString(3,dob.toLowerCase());
        ps.setString(4,tf3.toLowerCase());
        //ps.setString(6,tfn.toLowerCase());
        //ps.setString(7,tf4.toLowerCase());
        //ps.setString(8,tf5.toLowerCase());
        ps.setString(5,af1.toLowerCase());
        ps.setString(6,af2.toLowerCase());
        ps.setString(7,tf6.toLowerCase());
        ps.setInt(8,tf8); 
	ps.setString(9,tf7);
        ps.setString(10,tf9.toLowerCase());
        ps.setString(11,tf10);
	ps.setString(12,tf11);   	    
	ps.setString(13,tf12);
        ps.setInt(14,cno);
ps.execute();
     ...
     /* if(t1.getText()=="" || t2.getText()=="" || t3.getText()=="" || tn.getText()=="" || 
             r1.getLabel()==""||r2.getLabel()==""|| t4.getText()==""|| t5.getText()==""||
            a1.getText()==""||a2.getText()==""||t6.getText()==""||t7.getText()=="" ||
           t8.getText()==""||t9.getText()==""||t10.getText()=="")
